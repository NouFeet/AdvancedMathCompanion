...
/*!50001 VIEW `expression_list` AS select `expressions`.`expression_id` AS `expression_id`,`expressions`.`expression` AS `expression`,`expressions`.`answer` AS `answer`,`expressions`.`date` AS `date`,`types`.`type` AS `type`,`complexities`.`name` AS `name`,`complexities`.`description` AS `description` from (((`expressions` join `expression_to_types` on((`expressions`.`expression_id` = `expression_to_types`.`expression_id`))) join `types` on((`expression_to_types`.`type_id` = `types`.`type_id`))) join `complexities` on((`types`.`complexity` = `complexities`.`complexity_id`))) order by `expressions`.`expression_id` */;
...
BEGIN
	DELETE e, et FROM expressions e
    INNER JOIN expression_to_types et
    WHERE e.expression_id = expression_id AND e.expression_id = et.expression_id;
END ;;
DELIMITER ;
...
BEGIN
    ...
    (DEFAULT, expression, answer, date);
    
    INSERT INTO expression_to_types VALUES
    (LAST_INSERT_ID(), type_id);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
